...
import { updateDiscountGroup } from '../actions'
...
export function EditDiscountGroupDialog({ group, open, onOpenChange }: Props) {
  const router = useRouter()
  const [isPending, startTransition] = useTransition()
  const [formData, setFormData] = useState({
    name: group.name,
    discount_rate: group.discount_rate.toString(),
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const res = await updateDiscountGroup(group.id, {
        name: formData.name,
